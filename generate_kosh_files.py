...
def generate_all_kosh_files(path_to_freedict):
    for item in os.listdir(path_to_freedict):
        if os.path.isdir(os.path.join(path_to_freedict, item)):
            # we create a .kosh file for each dict
            generate_dot_kosh(path_to_freedict, item)
            generate_mapping(path_to_freedict, item)
